...
let baseUrl = "https://vegamovies.catering", hubDomain = 'https://hubcloud.foo', vcDomain = 'https://vcloud.zip';
let domainCache = null, domainCacheTime = 0, count1080p = 0;
...
const fixUrl = url => !url ? '' : url.startsWith('https://') ? url : url.startsWith('http://') ? 'https://' + url.slice(7) : url.startsWith('//') ? 'https:' + url : baseUrl + (url.startsWith('/') ? '' : '/') + url;
...
function makeStream(_, title, url, quality, headers, mediaInfo, fallbackQ = 'HD') {
  ...
  return {
    ...
    behaviorHints: { notWebReady: true, proxyHeaders: { request: headers || { "Referer": baseUrl + "/" } } }
  };
}
...
async function refreshDomains() {
  if (domainCache && Date.now() - domainCacheTime < 4 * 60 * 60 * 1000) return;
  try {
    const d = await fetchJson(DOMAINS_URL, {}, 8000);
    if (d) { domainCache = d; domainCacheTime = Date.now(); if (d.vegamovies) baseUrl = d.vegamovies; if (d.hubcloud) hubDomain = d.hubcloud; if (d.vcloud) vcDomain = d.vcloud; }
  } catch (e) { }
}
...
async function searchByTitle(query, year) {
  if (!query) return [];
  const d = await fetchJson(`${baseUrl}/search.php?q=${encodeURIComponent(query + (year ? ' ' + year : ''))}&page=1&per_page=15`, { headers: { ...mobileHdrs(), 'Accept-Encoding': 'identity' } });
...
async function fetchPostContent(postId, link) {
  ...
  try {
    const r = await fetchSafe(`${baseUrl}/wp-json/wp/v2/posts/${postId}`, { headers: mobileHdrs() }, 15000);
    if (r && r.ok) {
  ...
  try {
    const $ = await fetchHtml(link ? fixUrl(link) : `${baseUrl}/?p=${postId}`, { headers: mobileHdrs() });
...
async function extractSingleVc(vcUrl, referer, targetSeason, targetEp, label, fallbackQ, mediaInfo) {
  ...
  const isHub = lower.includes('hubcloud'), latestBase = isHub ? hubDomain : vcDomain, cur = origin(vcUrl);
  const newUrl = (cur !== latestBase && (vcUrl.includes('vcloud') || vcUrl.includes('hubcloud'))) ? vcUrl.replace(cur, latestBase) : vcUrl;

  const $ = await fetchHtml(newUrl, { headers: { ...mobileHdrs(), 'Referer': referer || baseUrl + '/', 'Cookie': 'xla=s4t' }, redirect: 'manual' });
  if (!$) return streams;
...
async function loadStreamsFromUrl(url, label, quality, referer, targetSeason, targetEp, mediaInfo) {
  ...
  const $ = await fetchHtml(url, { headers: { ...mobileHdrs(), 'Referer': referer || baseUrl + '/' }, redirect: 'manual' });
  if (!$) return [];
  const streams = [], tasks = [];
...
async function extractFromPost(post, label, isTv, targetSeason, targetEp, mediaYear) {
  try {
    ...
    const results = await Promise.all(links.map(l => loadStreamsFromUrl(l.href, l.label || (seasonLabel + '[' + l.quality + ']'), l.quality, baseUrl + '/', targetSeason, targetEp, mediaInfo).catch(() => [])));
    return results.flat();
  } catch (e) { return []; }
}
