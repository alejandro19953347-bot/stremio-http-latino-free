...
// Simple in-memory cache for stream results
...
async function resolveStreams(tmdbId, mediaType, season, episode, title) {
    ...
    if (cached && (Date.now() - cached.ts) < CACHE_TTL_MS) {
        console.log(`[Bridge] Cache HIT for ${cacheKey} (${cached.streams.length} streams, ${Math.round((Date.now()-cached.ts)/1000)}s old)`);
    ...
    const TIMEOUT_MS = 10000; // 10s per provider (was 20s)
    ...
    const results = await Promise.race([
        ...
        new Promise((resolve) => setTimeout(() => {
            console.log(`[Bridge] Global timeout reached, returning partial results`);
            resolve([]);
        }, 12000))
    ]);
    ...
    // Cache the result
    streamCache.set(cacheKey, { streams: unique, ts: Date.now() });
...
function toStremioStreams(rawStreams) {
    return rawStreams.map(s => {
        ...
        let isDirect = lowerUrl.includes(".m3u8") || lowerUrl.includes(".mp4") || lowerUrl.includes("/hls/") || lowerUrl.includes("/stream/") || lowerUrl.includes("/master.");
